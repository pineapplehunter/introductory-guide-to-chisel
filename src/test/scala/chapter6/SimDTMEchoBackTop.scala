...
class SimDTMEchoBackTopBundle extends BaseSimDTMBundle {
...
class SimDTMEchoBackTop(p: SimpleIOParams,
  ...
  val io = IO(new SimDTMEchoBackTopBundle())

  // UART送信処理
  val durationCount = round(clockFreq.toDouble / baudrate).toInt

  val m_uart_tx = Module(new Ctrl(UartTx, durationCount))
  val r_data_count = RegInit(0.U(log2Ceil(testData.length + 1).W))
  val reg_io = m_uart_tx.io.reg.asInstanceOf[FIFOReadIO[UInt]]
